...
class Stream {
    ...
    push(...bytes) {
        ...
        const len = this.data.push(...Array.from(bytes).flat(10));
        this.#updateOutput();
    ...
    // const update_cursor = () => {
    //     const m =
    //         /\n?([^\n]*?)$/s.exec(this.content_element.innerText)[1]
    //             .length + 1;
    //     this.cursor_element.textContent = evt.key === "Enter"
    //         ? "_"
    //         : "_".trimStart().padStart(m, " ");
    // };
    async read() {
        ...
        console.log(this.data.slice(0).join(""));
...
class Terminal {
    ...
    constructor(options = {}) {
        ...
        this.stdout = new Stream(this.#stdout_buf, "w", {
            update_output: () => {
                this.content_element.innerText = this.#stdout_buf.join("");
                this.update_cursor();
            },
        });
        ...
        this.content_element.addEventListener(
            ...
            (evt) => {
                ...
                if (this.#composing) {
                    ...
                    if (
                        key.length === 1 &&
                        compose_counter[compose_buf += key]
                    ) return;
                ...
                if (!this.raw_mode) {
                    console.log(
                        this.#stdout_buf.length,
                        this.#stdin_buf.length,
                    );
                    this.content_element.innerText = this.#stdout_buf.join("") +
                        this.#stdin_buf.join("");
                ...
                this.stdout.push(this.#stdin_buf.join(""));
                this.stdin.notify();
                this.update_cursor();
            },
        );
    ...
    update_cursor() {
        const m = /\n?([^\n]*?)$/s.exec(this.content_element.innerText)[1]
            .length + 1;
        this.cursor_element.textContent = "_".trimStart().padStart(m, " ");
    }
}
new Terminal({
